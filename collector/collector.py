...
def execute_remote_script(ssh_client):
    ...
    try:
        ...
        # Créer un fichier temporaire sur le serveur distant
...
def collect_from_server(target):
    ...
    try:
        ...
        if not metrics:
            logger.warning(f"⚠️  Aucune métrique collectée pour {ip}")
...
def collect_all_targets():
    ...
    try:
        ...
        if not targets:
            logger.warning("⚠️  Aucun serveur actif à surveiller")
            return
...
def main():
    ...
    logger.info(f"⏱️  Intervalle de collecte: {Config.POLL_INTERVAL}s")
    ...
    try:
        while True:
            ...
            logger.info(f"⏸️  Pause de {sleep_time:.1f}s avant la prochaine collecte")
